...
# echo $PORT
echo ${prodApplicationURL}${applicationURI}

# if [[ ! -z "$PORT" ]];
# then

response=$(curl -s ${prodApplicationURL}${applicationURI})
http_code=$(curl -s -o /dev/null -w "%{http_code}" ${prodApplicationURL}${applicationURI})
